...
- **How much have I done?** A **progress bar** shows six bands — not started, met, one dot, two dots, learned, mastered — captioned on the session screen with the selection it covers (*"Food · B1"*, or *"all words"*), since the pickers are not in sight there; in Settings, where they are, it is simply *"Progress in this selection"*. Its legend names each band in the same dots the cards draw — no words, just the ladder and a count. It appears in **Settings → What** and on the session start screen. The header (top-right) leads with your **learned** total (retired words included, so it never drops) and how many are **in progress**. The deck's total size is on the Words tab.
- The **Words** tab is filterable by **Level**, **Topic** and **Progress**, on top of the search box. The Progress row is **All** followed by the dot ladder itself, climbing left to right from three empty dots to three gold ones — drawn by the same code as the cards, with no labels; a line underneath names the step you picked and what it means (*"Met — seen, but never recalled"*). Only bands that contain words under the other filters are shown. **Tap any word** to expand its example sentences, level, and topic, then **✏️ Edit** to change any field, including moving it to a different topic (typing an existing topic name, in any case, folds it into that topic). Edits stay in this browser.
